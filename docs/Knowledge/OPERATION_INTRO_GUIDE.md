...
本文基于 `README.md` 和 `MANUAL.md` 整理，面向第一次接触 OpenMelon 的用户、测试人员、产品/研发同学和演示讲解场景。目标是用一份更顺滑的内容说明：OpenMelon 是什么、能做什么、如何启动、进入系统后该按什么顺序操作，也补充当前版本的运行配置中心使用方式。
...
如果当时还没有准备 `.env`，也可以稍后通过“设置 -> 运行配置”执行初始化。
...
- `热更新`：保存后影响后续新请求，不会切换已经在跑的任务。
- `需重启`：已经写入 `.env`，但涉及路径、数据库、向量库主连接或启动期资源，仍要重启后端。
...
当前版本也支持在“设置 -> 运行配置 -> Provider 管理”里维护自定义 Provider 模板。这里管理的是模板库，不会自动替换当前 `.env`；真正的运行参数仍要回到主模块 LLM 分组保存。

### 6.1.1 运行配置中心建议理解

推荐把设置页中的“运行配置”理解成当前项目的统一配置面板：

1. 没有 `.env` 时，可先做最小初始化或从模板初始化。
2. 有 `.env` 后，优先在页面里调整主模块 LLM、Embedding、检索和日志生命周期参数。
3. 需要沉淀厂商接入方案时，在 `Provider 管理` 中维护模板。
4. 调整后先看字段是 `热更新` 还是 `需重启`，避免误判生效范围。
