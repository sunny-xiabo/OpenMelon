...
- **可视化工作流编排**：Dify 风格的 DAG 工作流构建与执行系统，支持 13 种节点类型（LLM、HTTP、代码执行、条件分支、知识检索等）；画布式拖拽编排、变量池作用域解析、asyncio 并行执行；SSE 实时事件流、执行队列并发控制；支持 JSON/YAML DSL 导入导出及模板库。
...
| **8** | **工作流** | 可视化构建 DAG 工作流，拖拽节点编排业务流程，执行并查看实时日志 |
...
- **工作流**：![工作流页面](docs/screenshots/workflow-page.png)
