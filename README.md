...
如果你要维护 Prompt Hub 配置，请直接看 [`MANUAL.md`](MANUAL.md) 里的 `7.6 Prompt & Skill Hub 管理` 和 `7.7 模板与技能编写规范`。
...
| 页面 | 主要用途 | 适合什么时候打开 |
|------|------|------|
| 导入管理 | 上传文档、查看索引状态、重建索引 | 第一次使用系统，或文档有更新时 |
| 问答 | 基于图谱和向量检索直接提问 | 想快速查知识点、流程、影响范围时 |
| 图谱总览 | 查看实体和关系网络 | 想理解模块结构、上下游关系时 |
| 测试用例生成 | 用 AI 生成、评审、导出测试用例 | 想从需求或文档直接产出测试用例时 |
| 覆盖率视图 | 查看模块功能覆盖和风险模块 | 想定位哪些模块缺测试时 |
| 设置 | 管理节点类型、Prompt Hub 等系统配置 | 想调整显示规则、模板、技能和生成策略时 |

推荐的新手路径是：

1. 先去「导入管理」喂给系统一批资料。
2. 再去「问答」和「图谱总览」确认知识已经可查。
3. 然后进「测试用例生成」体验 Prompt Hub 和 AI 用例链路。
4. 最后用「覆盖率视图」看哪些模块还需要补测试。
...
- **会话管理**：自动以首次提问生成会话标题，支持重命名、删除确认、相对时间显示；直接发消息自动创建会话
...
- 支持通过「设置 > Prompt Hub」管理生成模板和专项技能，生成页动态读取并支持默认回退
- Prompt Hub 管理页支持技能分类中文化、自定义分类、搜索筛选，以及模板/技能分栏管理
...
- 指标卡片（带图标与趋势标签）+ 环形总览图 + 覆盖率排行（排名 + hover 提示）+ 模块明细表
- 表格支持 sticky header 滚动、按覆盖率/功能数/用例数排序、模块搜索和风险过滤
- 点击行展开查看功能列表与关联用例，支持一键导出 CSV 报告
