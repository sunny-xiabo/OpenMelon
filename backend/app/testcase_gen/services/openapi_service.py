...
class OpenAPIService:
    ...
    def parse_openapi_file(self, file_path: str) -> Dict[str, Any]:
        ...
        返回:
            解析后的API结构信息
        """
        logger.info(f"开始解析OpenAPI文件: {file_path}")
        try:
            # 根据文件扩展名选择解析方法
            ...
            logger.info(f"OpenAPI解析成功 - 标题: {parsed_api['info'].get('title')}, 路径数: {len(parsed_api['paths'])}")
            ...
            logger.error(f"OpenAPI文件解析失败: {str(e)}", exc_info=True)
