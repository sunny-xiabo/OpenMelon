...
6. 如需接口执行，进「API 自动化」导入接口文档并运行 API 用例
...
| 覆盖率分析 | 从图谱里的功能和用例关系计算覆盖情况 | 看总览、排序模块、定位高风险模块 | 测试负责人、项目负责人 |
| API 自动化 | 将接口文档转成可执行 API 用例，支持运行、报告、AI 修复和知识沉淀 | 导入 OpenAPI/Postman/HAR/文档、生成 API DSL、配置环境、执行、确认沉淀 | 测试、研发、接口负责人 |
| 设置 | 管理系统行为和显示规则 | 调整节点类型、管理 Prompt Hub 模板/技能 | 管理员、维护者 |

建议的理解顺序：

1. 先看「导入管理」，理解资料是怎么进入系统的。
2. 再看「问答」和「图谱总览」，理解知识是怎么被查询和展示的。
3. 然后看「测试用例生成」，理解业务文档如何转成测试资产。
4. 再看「API 自动化」，理解接口文档如何转成可执行用例和执行知识。
5. 最后看「覆盖率分析」和「设置」，理解管理和治理能力。
...
![覆盖率视图页面](docs/screenshots/coverage-page.png)
...
- [8. API 自动化](#8-api-自动化)
  - [8.5 API DSL 是什么](#85-api-dsl-是什么)
  - [8.7 项目策略与安全边界](#87-项目策略与安全边界)
  - [8.8 AI 修复与历史知识复用](#88-ai-修复与历史知识复用)
  - [8.9 知识沉淀质量门](#89-知识沉淀质量门)
- [9. 覆盖率分析](#9-覆盖率分析)
- [10. 导入管理](#10-导入管理)
- [11. 会话管理](#11-会话管理)
- [12. 节点类型配置](#12-节点类型配置)
- [13. 系统监控与日志](#13-系统监控与日志)
- [14. 故障排查](#14-故障排查)
...
API 自动化模块用于把接口文档转换为可执行的 API 测试用例，并将可信的执行结果、失败原因和修复经验沉淀回知识库。它和「导入管理」「测试用例生成」「覆盖率分析」是同一条测试资产闭环的一部分。

### 8.1 模块定位

| 能力 | 说明 |
|------|------|
| 接口资产导入 | 支持 OpenAPI / Swagger UI、Postman Collection、Apifox/ApiPost、HAR、Markdown/Excel/Word 接口文档、纯接口地址列表和普通 HTML 页面 |
...
| AI 辅助 | 支持 AI DSL 补全、失败诊断、修复补丁生成和低风险受控自动修复重跑 |
...
```mermaid
flowchart TD
    Import["导入 API 文档"] --> Asset["接口资产列表"]
    Asset --> Select["选择接口"]
    Select --> DSL["生成 API DSL"]
    DSL --> Config["配置项目与环境"]
...
| 项目与环境区 | 配置项目策略、环境类型、Base URL、Header、变量和超时 |
