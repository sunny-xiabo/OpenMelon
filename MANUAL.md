...
| 数据仪表盘 | 汇聚全链路的覆盖率分析与自动化测试健康度大屏 | 查看覆盖率总览、API执行通过率、定位高风险模块 | 测试负责人、项目负责人 |
| 设置 | 管理系统行为和显示规则 | 调整节点类型、管理 Prompt Hub 模板/技能、环境变量 | 管理员、维护者 |
...
5. 看「API 自动化」，理解接口文档如何转成可执行用例并运行。
6. 最后看「数据仪表盘」和「设置」，掌握整个测试生态的全局统计与底层管控。
