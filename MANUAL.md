...
| 设置 | 统一配置中心，左侧分区导航切换多个子页 | 节点类型配置、运行配置、健康检查、Prompt Hub、项目与环境、治理中心、AI/RAG 观测、日志中心 | 管理员、维护者 |
...
![健康检查](docs/screenshots/system-health-page.png)
...
| 功能 | 说明 |
|------|------|
| 配置导出 | 将当前运行配置（含 LLM、Embedding、检索、Reranker 等全部参数）导出为 JSON 文件 |
| 配置导入 | 从 JSON 文件恢复配置，适合环境迁移或批量初始化 |
| 版本历史 | 每次保存配置自动记录版本快照，支持查看历史版本和一键回滚到任意历史版本 |
| LLM 端点验证 | 保存配置时自动验证 LLM API 端点可达性，端点不通时页面提示错误并不写入 `.env` |
...
点击来源标签中的图谱引用，可在「图谱总览」页面高亮对应节点及关联关系，便于快速定位知识上下文。

**消息操作**：每条回答右下方提供快捷操作按钮：

| 操作 | 说明 |
|------|------|
| 复制 | 一键复制回答内容到剪贴板 |
| 重试 | 用相同问题重新发起检索和生成，适合对回答质量不满意时快速重试 |
| 点赞 / 点踩 | 对回答质量进行反馈，反馈数据用于后续优化 |

**图片多模态查询**：在提问输入框中可通过附件按钮上传图片（截图、UI 原型、流程图等），系统会将图片与文字问题一起发送给支持视觉理解的 LLM 进行联合推理，适合基于截图描述 UI 问题或解释图表内容。
...
在图谱页面右上角工具栏点击「导出 PNG」，系统会将当前视口内的图谱渲染为高清 PNG 图片并触发浏览器下载。导出内容与当前缩放、筛选和高亮状态一致，适合在文档或报告中直接引用当前图谱快照。

### 6.7 最短路径查询

在图谱页面点击「路径查询」按钮，依次选择两个节点，系统会在 Neo4j 中计算并高亮两节点间的最短路径（含中间节点和关系边）。适用于分析两个实体之间的依赖链路或影响路径。
...
1. 左侧：选择生成模式、上传文件/输入文本、填写需求；填写过程中表单内容自动保存为草稿，刷新页面后可恢复
2. 点击「生成测试用例」按钮
3. 右侧实时展示：
   - 阶段时间线（需求分析 → 用例生成 → 用例评审）
   - 每个阶段的 Markdown 流式输出
   - 完成后的结果统计卡片
4. 生成过程中可点击「取消」按钮中止当前生成任务，已产出的部分结果仍然保留
5. 生成完成后可以：
   - 切换列表视图/导图视图
   - 按模块、优先级筛选
   - 直接在页面内行编辑用例标题、步骤、优先级等字段
   - 点击「存入向量库」直接且永久保存
   - 点击「用例导出」可选择导出结构化 Excel 表格文件、原生脑图格式文件（.xmind）或 Markdown 格式文件下载
